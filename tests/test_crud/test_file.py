...
async def test_create(db_pool: DBPool, user: User):
    path = Path("a/b/f")
    await crud.file.create_folder(db_pool, user.namespace.path, path.parent)

    folder = await crud.file.get(db_pool, user.namespace.path, path.parent)
    assert folder.size == 0

    await crud.file.create(db_pool, user.namespace.path, path, size=32)
...
async def test_create_folder_but_folder_exists(db_pool: DBPool, user: User):
    await crud.file.create_folder(db_pool, user.namespace.path, "folder")
...
async def test_exists(db_pool: DBPool, user: User):
    await crud.file.create(db_pool, user.namespace.path, "file")
    assert await crud.file.exists(db_pool, user.namespace.path, "file")
...
async def test_delete_file_but_it_not_exists(db_pool: DBPool, user: User):
    with pytest.raises(errors.FileNotFound):
...
async def test_get(db_pool: DBPool, user: User):
    await crud.file.create(db_pool, user.namespace.path, "file")
    file = await crud.file.get(db_pool, user.namespace.path, "file")
...
async def test_move_but_paths_are_recursive(db_pool: DBPool, user: User):
    namespace = user.namespace.path
    with pytest.raises(AssertionError) as excinfo:
        await crud.file.move(db_pool, namespace, "a/b", "a/b/b")
