...
if TYPE_CHECKING:
    ...
    from app.infrastructure.database.tortoise.repositories import AuditTrailRepository
...
class TestSave:
    ...
    async def test_saving_asset(
        ...
        assets = await models.AuditTrailAsset.filter(audit_trail_id=saved_trail.id)
        assert len(assets) == 1
        assert assets[0].file_id == file.id  # type: ignore[attr-defined]
