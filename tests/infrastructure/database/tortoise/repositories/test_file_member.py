...
if TYPE_CHECKING:
    ...
    from app.infrastructure.database.tortoise.repositories import FileMemberRepository
...
async def _list_members(file_id: UUID) -> list[FileMember]:
    objs = await (
        models.FileMember
...
class TestListByFileIdBatch:
    async def test(
        ...
        # WHEN
        result = await file_member_repo.list_by_file_id_batch(
