...
class TestDeleteBatch:
    async def test(self, fs_storage: FileSystemStorage, file_factory: FileFactory):
        ...
        # WHEN
        await fs_storage.delete_batch(keys=["user/f.txt", "user/f (1).txt"])
        # THEN
...
class TestDownloadDir:
    ...
    async def test_including_only_specified_paths(
        ...
        # WHEN
        chunks = fs_storage.downloaddir("user/a", include_keys=keys_to_include)
...
class TestIterdir:
    ...
    async def test_when_does_not_include_broken_symlinks(
        ...
        # THEN
        assert (len(files)) == 1
        assert files[0].path == "user/x.txt"
